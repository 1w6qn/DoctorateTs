...
Game-data update (`scripts/update-data.ts`) clones/pulls `ArknightsGameData` into the repo root (gitignored); excel/playerdata 类型均由 CS 反编译源生成（不再依赖 OpenArknightsFBS/FBS）。
