...
/**
 * 公招 mock 的干员行（recruit 只读 name/rarity/position/profession/tagList，故为窄视图；
 * 真实 `CharacterData` 的其余字段本例不涉及）
 */
interface MockRecruitCharRow {
  name: string;
  rarity: string;
  position: string;
  profession: string;
  /** 缺失即 undefined——覆盖「无标签干员」这条既有回归用例（char_008） */
  tagList?: string[];
}

// 公招数据 mock（参考 data/excel/gacha_table.json 结构）
const excelMock = vi.hoisted(() => {
  /** 干员表（带索引签名，门面 charData 按 charId 取行） */
  const characterTable: Record<string, MockRecruitCharRow> = {
...
vi.mock("@utils/random", () => ({
  randomInt: vi.fn(() => 1),
  randomSample: vi.fn(<T>(arr: readonly T[], n: number): T[] => arr.slice(0, n)),
  randomChoice: vi.fn(<T>(arr: readonly T[]): T => arr[0]),
  randomChoices: vi.fn(<T>(arr: readonly T[], _w: readonly number[], n: number): T[] =>
    Array(n).fill(arr[0]),
  ),
}));
...
import type { GachaResult } from "@game/kernel/model";
import { RecruitManager } from "@game/modules/gacha/recruit";
...
describe("RecruitManager 核心方法", () => {
  ...
  it("refreshTags 应刷新槽位标签", async () => {
    ...
    vi.spyOn(RecruitTools, "refreshTagList").mockResolvedValue([2, 4, 6, 8, 10]);
    const manager = new RecruitManager(asPlayerManager(mockPlayer), mockTrigger);
  ...
  it("cancel 应重置槽位状态", async () => {
    const { RecruitTools } = await import("@game/modules/gacha/recruit");
    vi.spyOn(RecruitTools, "refreshTagList").mockResolvedValue([9, 9, 9, 9, 9]);
  ...
  it("normalGacha 应开始招募并消耗招募券", async () => {
    const { RecruitTools } = await import("@game/modules/gacha/recruit");
    vi.spyOn(RecruitTools, "refreshTagList").mockResolvedValue([1, 2, 3, 4, 5]);
    const manager = new RecruitManager(asPlayerManager(mockPlayer), mockTrigger);
  ...
  it("finish 应结算招募并出干员", async () => {
    const { RecruitTools } = await import("@game/modules/gacha/recruit");
    vi.spyOn(RecruitTools, "generateValidTags").mockResolvedValue(["char_001", [1]]);
    vi.spyOn(RecruitTools, "refreshTagList").mockResolvedValue([1, 2, 3, 4, 5]);
    const manager = new RecruitManager(asPlayerManager(mockPlayer), mockTrigger);
    // 准备进行中槽位
    mockPlayer._playerdata.recruit!.normal.slots["0"] = {
    ...
    let captured!: { charId: string; from: EventMap["char:get"][1] };
    mockTrigger.on("char:get", ([charId, from, cb]) => {
      captured = { charId, from };
      // 夹具只回填 charId：管理器仅透传该结果、不读其余字段（运行期载荷与原用例一致）
      cb?.({ charId } as GachaResult);
    });
