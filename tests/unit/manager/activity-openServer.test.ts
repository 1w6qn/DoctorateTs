...
vi.mock("@excel/excel", () => ({
  default: {
    // 门面方法读取的三张表显式声明为空表（工厂对象无上下文类型，`this` 即字面量自身）；
    // 与「键不存在的旧 mock」运行期等价（`?.` 链同样取到 undefined）
...
/** `@excel/excel` 端口在本用例只被读写 schedule 的窗口字段（窄视图，避免 cast） */
interface OpenServerExcelView {
  OpenServerTable: { schedule: { id: string; startTs: number; endTs: number }[] };
}
...
describe("OpenServerManager", () => {
  ...
  it("dailyRefresh 应推进连续登录并记录签到", async () => {
    const manager = new OpenServerManager(asPlayerManager(mockPlayer), mockTrigger);
  ...
  it("活动窗口外一律不可领取", async () => {
    const excel: OpenServerExcelView = (await import("@excel/excel")).default;
  ...
  it("openServer 状态缺失时兜底初始化（原实现直接 500）", async () => {
    delete (mockPlayer._playerdata as Partial<PlayerDataModel>).openServer;
    const manager = new OpenServerManager(asPlayerManager(mockPlayer), mockTrigger);
