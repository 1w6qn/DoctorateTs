...
日期：2026-08-25（实施完成：2026-08-26，见 design-spec.md §11.13）
状态：已实现（6 批全部落地，基建 12 文件 298 测试通过，tsc 干净）
...
### 9.2 中枢杂项（G17/G18）
- `buyLabor`：控制中枢等级 ≥ `apToLaborUnlockLevel`(4) 时，官方路径 `1 AP → 2 劳动力`（扣 `status.ap`，钳制 `labor.maxValue`，已实现）；未达级保留源石兼容路径（低等级可用），文档记录。
- **G18 副手信赖（延后）**：实施中评估后延后——wiki 未给氛围折算公式（低置信），且需新增待领取实体/领取端点（进驻≤100/中枢副手≤500/楼层副手≤125，未收取覆盖），需真实存档/抓包校准后单独立项（已记录 design-spec §11.13 已知限制）。
...
- 实施中确认的延后项：副手信赖定时结算（G18，需真实存档校准）；患难之交 dorm_exchangeAp（需进驻顺序追踪）；自律/嗜睡/慵懒（数据表无匹配 buff）；尤里卡订单（数据表无对应 buff）。
