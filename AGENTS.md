...
- `data/excel/*.json` — generated by `scripts/hotupdate-excel.py` (official CDN hot-update: FBO 解码或 AES-CBC JSON 解密 + camelCase + 枚举字符串转换); **`npm run update` 触发管线并重新生成类型**. Manual excel tweaks get clobbered. 部分表（range/player_avatar/roguelike/sandbox/uniequip_data/handbook/tech_buff）为 AES-CBC 加密 JSON，直接用 `MASK_V2` 解密，非 FBS。
- `2221.js` — compiled Frida artifact, never edit.

Game-data update (`scripts/update-data.ts`) 调用官方热更管线 `scripts/hotupdate-excel.py --download --decode --convert`（无 ArknightsGameData 仓库依赖）；excel/playerdata 类型均由 CS 反编译源生成（不再依赖 OpenArknightsFBS/FBS）。
