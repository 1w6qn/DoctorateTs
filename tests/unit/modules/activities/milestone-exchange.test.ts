import { describe, it, expect, vi } from "vitest";

/** 本用例读到的 excel 行形状（只经 ?. 读字段，故全部可选） */
interface ExcelItemRow {
  name?: string;
}
interface ExcelCharRow {
  name?: string;
}
interface ExcelStageRow {
  stageType?: string;
}

/** excel 门面替身视图（与本文件 vi.mock 工厂形状一致） */
interface ExcelFacadeMock {
  ItemTable: { items: Record<string, ExcelItemRow> };
  CharacterTable: Record<string, ExcelCharRow>;
  StageTable: { stages: Record<string, ExcelStageRow> };
  getItem(id: string): ExcelItemRow | undefined;
  itemName(id: string): string;
  makeItem(id: string, count: number, type?: string): { id: string; count: number; type?: string };
  charData(charId: string): ExcelCharRow | undefined;
  stageData(stageId: string): ExcelStageRow | undefined;
}

/**
 * 活动商店兑换（handleExchangeActivityShopItem）
 *
 * 修复（2026-09-09）：count 必须为正整数 —— 原实现 `body.count || 1` 直接透传负数，
 * recordPurchase 的 `existing.count += count` 会把已购数量减回去（甚至变负），
 * 从而绕过活动商店的限购判定再买一轮。
 */
vi.mock("@excel/excel", () => {
  const facade: ExcelFacadeMock = {
    getItem(id: string) { return this.ItemTable?.items?.[id]; },
    itemName(id: string): string { return this.getItem(id)?.name ?? id; },
    makeItem(id: string, count: number, type?: string) { return type ? { id, count, type } : { id, count }; },
    charData(charId: string) { return this.CharacterTable?.[charId]; },
    stageData(stageId: string) { return this.StageTable?.stages?.[stageId]; },
    ItemTable: { items: {} },
    CharacterTable: {},
    StageTable: { stages: {} },
  };
  return { default: facade };
});

import { handleExchangeActivityShopItem } from "@game/modules/activities/milestone/logic";
import type { ExchangeActivityShopItemRequest } from "@game/modules/activities/shared/activity";
import type { PlayerDataModel } from "@game/kernel/playerdata";
import { asPlayerManager, mockPlayerData, type MockSeed } from "../../../helpers";

function makePlayer(tshop: MockSeed<PlayerDataModel["tshop"]> = {}) {
  return mockPlayerData({
    tshop,
    pushFlags: {},
  });
}

describe("活动商店兑换 count 校验", () => {
  it("正数 count 正常记录购买", async () => {
    const player = makePlayer({});
    await handleExchangeActivityShopItem(asPlayerManager(player), {
      shopId: "s1",
      goodId: "g1",
      count: 2,
    } satisfies ExchangeActivityShopItemRequest);
    expect(player._playerdata.tshop["s1"].info).toEqual([{ id: "g1", count: 2 }]);
  });

  it("负数 count 应拒绝（不写购买记录）", async () => {
    const player = makePlayer({
      s1: { coin: 0, info: [{ id: "g1", count: 3 }], progressInfo: {} },
    });
    await handleExchangeActivityShopItem(asPlayerManager(player), {
      shopId: "s1",
      goodId: "g1",
      count: -2,
    } satisfies ExchangeActivityShopItemRequest);
    // 原实现会把 3 减成 1（绕过限购）；现保持 3 不变
    expect(player._playerdata.tshop["s1"].info).toEqual([{ id: "g1", count: 3 }]);
  });

  it("非整数/零 count 应拒绝", async () => {
    const player = makePlayer({});
    await handleExchangeActivityShopItem(asPlayerManager(player), { shopId: "s1", goodId: "g1", count: 0 } satisfies ExchangeActivityShopItemRequest);
    // 拒绝即完全不写档（连 shop 条目都不创建）
    expect(player._playerdata.tshop["s1"]).toBeUndefined();
    const player2 = makePlayer({});
    await handleExchangeActivityShopItem(asPlayerManager(player2), { shopId: "s1", goodId: "g1", count: 1.5 } satisfies ExchangeActivityShopItemRequest);
    expect(player2._playerdata.tshop["s1"]).toBeUndefined();
  });
});
