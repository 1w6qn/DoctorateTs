...
router.post("/selectEventChoice", validateBody(selectEventChoiceSchema), async (req, res) => {
  const player = getPlayer();
...
router.post("/confirmPassTarget", validateBody(confirmPassTargetSchema), async (req, res) => {
  const player = getPlayer();
