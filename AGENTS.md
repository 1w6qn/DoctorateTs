...
- `data/excel/*.json` — generated by `scripts/hotupdate-excel.py` (official CDN hot-update FBO 解码 + camelCase + 枚举字符串转换); **`npm run update` 触发管线并重新生成类型**. Manual excel tweaks get clobbered. 无 FBS schema 的表（range/player_avatar/roguelike/sandbox/uniequip_data 等）保持本地快照。
