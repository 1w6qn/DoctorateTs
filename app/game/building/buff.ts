 ...
 * 其次带符号的（+0.15），最后无符号数字；无标签返回 null。
 */
export function parseVupValue(desc?: string | null): number | null {
  const re = /<@cc\.vup>\s*([+-]?\d+(?:\.\d+)?)\s*(%?)/g;
  const all: Array<{ v: number; hasPct: boolean; signed: boolean }> = [];
...
/**
 * 心情消耗值：取描述"消耗"之后第一个 <@cc.vup>/<@cc.vdown> 数值（带符号，点/小时）。
...
export function parseMoodCostValue(desc?: string | null): number | null {
  ...
  const m = /<@cc\.(?:vup|vdown)>\s*([+-]?\d+(?:\.\d+)?)/.exec(
    (desc ?? "").slice(idx),
  );
...
export function buffValueForTarget(buff: any, targetRoom?: string | null): number {
  ...
  const v = parseVupValue(desc);
  if (v == null) return 0;
  if (/%/.test(desc)) return v / 100;
  if (targetRoom === "DORMITORY") return v;
  return 0;
}
