...
### 20.2 已实施优化（perf(auth): 存档原子写与防抖合并保存）
1. **原子写**（`savePlayerData`）：先写 `{uid}.json.tmp` 再 `rename`——避免写盘中断导致存档截断损坏（评估中发现 1.json 曾损坏）
2. **防抖合并**（`scheduleSave`/`flushSave`）：save 事件改为 500ms 防抖——窗口内多次变更只落盘一次（多玩家/高频请求场景每请求省 ~6.6ms 序列化）；`flushSave(uid)` 可显式立即保存（服务器关闭时调用）
