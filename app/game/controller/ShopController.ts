...
export class ShopController {
    ...
    buyHighGood(goodId: string, count: number): ItemBundle[] {
        let good = this.highGoodList.goodList.find(g => g.goodId === goodId)
        let item:ItemBundle
        if (!good?.progressGoodId) {
            ...
            let progressGood=this.highGoodList.progressGoodList[good!.progressGoodId]
            item=progressGood[this.shop.HS.progressInfo[good!.progressGoodId].order-1].item
            //TODO
        ...
        this._trigger.emit("useItems", [{ id: "4004", count: good!.price * count }])
        this._trigger.emit("gainItems", [item])
        return [item]
    }
    buyExtraGood(goodId: string, count: number): ItemBundle[] {
        let good = this.extraGoodList.goodList.find(g => g.goodId === goodId)
        let item = { id: good!.item.id, count: good!.item.count * count }
        ...
        this._trigger.emit("useItems", [{ id: "4006", count: good!.price * count }])
        this._trigger.emit("gainItems", [item])
        return [item]
    }
    ...
    buyEPGSGood(goodId: string, count: number): ItemBundle[] {
        let good = this.EPGSGoodList.goodList.find(g => g.goodId === goodId)
        let item = { id: good!.item.id, count: good!.item.count * count }
        ...
        this._trigger.emit("useItems", [{ id: "EPGS_COIN", count: good!.price * count }])
        this._trigger.emit("gainItems", [item])
        return [item]
    }
    buyREPGood(goodId: string, count: number): ItemBundle[] {
        let good = this.REPGoodList.goodList.find(g => g.goodId === goodId)
        let item = { id: good!.item.id, count: good!.item.count * count }
        ...
        this._trigger.emit("useItems", [{ id: "REP_COIN", count: good!.price * count }])
        this._trigger.emit("gainItems", [item])
        return [item]
    }
    buyClassicGood(goodId: string, count: number): ItemBundle[] {
        let good = this.classicGoodList.goodList.find(g => g.goodId === goodId)
        let item:ItemBundle
        if (!good?.progressGoodId) {
            item = { id: good!.item.id, count: good!.item.count * count }
            if (this.shop.CLASSIC.info.some(i => i.id === good!.goodId)) {
                this.shop.CLASSIC.info.find(i => i.id === good!.goodId)!.count += count
            } else {
                this.shop.CLASSIC.info.push({ id: good!.goodId, count: count })
            }
        }else{
            let progressGood=this.classicGoodList.progressGoodList[good!.progressGoodId]
            item=progressGood[this.shop.CLASSIC.progressInfo[good!.progressGoodId].order-1].item
            //TODO
    ...
    buyLMTGSGood(goodId: string, count: number): ItemBundle[] {
        let good = this.LMTGSGoodList.goodList.find(g => g.goodId === goodId)
        let item = { id: good!.item.id, count: good!.item.count * count }
        this._trigger.emit("useItems", [{ id: "LMTGS_COIN", count: good!.price.count * count }])
        //TODO
        this._trigger.emit("gainItems", [item])
        return [item]
    }
