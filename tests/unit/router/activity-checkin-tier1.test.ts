...
import type { ActivityTable_BasicData } from "@excel/types_excel_gen";
...
/**
 * 路由测试请求体视图（本文件各端点用到的字段集合）
 *
 * 被测路由按 zod schema 校验后只读取这几个字段；各端点按需取用子集，
 * 故统一声明为可选、不做判别联合。
 */
...
describe("第一档：纯领奖类活动真实发奖", () => {
  ...
  async function call(url: string, body: ActivityBody) {
  ...
  it("getActivityCheckInReward 已领取的 index 不再重复发奖", async () => {
    player._playerdata.activity = asModel<PlayerDataModel["activity"]>({
  ...
  it("getActivityCheckInReward：index 超过已过天数时被拒（原实现可一次领完全部 index）", async () => {
    ...
    excel.ActivityTable.basicInfo = asModel<Record<string, ActivityTable_BasicData>>({ act60sign: { startTime: today } }); // 活动首日
    await call("/getActivityCheckInReward", { activityId: "act60sign", index: 1 });
    ...
    expect(vi.mocked(res.send).mock.calls[1][0].items).toEqual([
  ...
  it("getActivityCheckInReward：同一自然日只能签到一次（读 lastTs）", async () => {
    const excel = (await import("@excel/excel")).default;
    const today = Math.floor(Date.now() / 1000);
    excel.ActivityTable.basicInfo = asModel<Record<string, ActivityTable_BasicData>>({ act60sign: { startTime: today - 5 * 86400 } });
    await call("/getActivityCheckInReward", { activityId: "act60sign", index: 0 });
    expect(vi.mocked(res.send).mock.calls[0][0].items).toHaveLength(1);
    ...
    await call("/getActivityCheckInReward", { activityId: "act60sign", index: 1 });
    expect(vi.mocked(res.send).mock.calls[2][0].items).toHaveLength(2);
  });

  it("checkinAllPlayer 签到：同样受天数门槛约束", async () => {
    const excel = (await import("@excel/excel")).default;
    const today = Math.floor(Date.now() / 1000);
    excel.ActivityTable.basicInfo = asModel<Record<string, ActivityTable_BasicData>>({ act1checkin: { startTime: today } }); // 活动首日
  ...
  it("checkinAllPlayer/syncBehaviorData 返回空增量（行为统计 TODO）", async () => {
    ...
    expect(vi.mocked(res.send).mock.calls[0][0]).toEqual({ playerDataDelta: {} });
  });
  ...
  it("excel 无配置时签到/登录/开关/对决均容错返回空奖励（不 500）", async () => {
    ...
    expect(vi.mocked(res.send).mock.calls[1][0].reward).toEqual([]);

    await call("/getSwitchOnlyReward", {
      activityId: "act_none",
      reward: "r1",
    });
    expect(vi.mocked(res.send).mock.calls[2][0].items).toEqual([]);
  });
