...
describe("act1vhalfidle（次生预案）路由", () => {
  ...
  function makePlayer(activity: any, extra: any = {}) {
    return mockPlayerData({
      activity: { HALFIDLE_VERIFY1: { act1vhalfidle: activity } },
      troop: { chars: {} },
      ...extra,
    } as any);
  }
  ...
  function halfIdle() {
    return player._playerdata.activity.HALFIDLE_VERIFY1.act1vhalfidle;
  }
  ...
  it("recruitNormal：随机任命按 poolTypeData 口径排除预留/联动未持有/低星", async () => {
    ...
    expect(res.send.mock.calls[0][0].ticketCount).toBe(0);
    const gained = Object.values(halfIdle().troop.chars).map((c: any) => c.charId);
...
expect(gained).not.toContain("char_link_missing");
  ...
  it("recruitDirect：特约任命按分档消耗（100/150）且可任命未持有干员", async () => {
    ...
    const chars = Object.values(halfIdle().troop.chars) as any[];
