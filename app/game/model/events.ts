 ...
 * 游戏事件系统接口定义（组合入口）
 *
 * 为降低中央契约的耦合，事件映射按领域拆分为多个独立文件：
 *   - `eventmaps/core.ts`      核心生命周期 / 对象变更 / 共享交易事件
 *   - `eventmaps/mission.ts`   任务系统事件
 *   - `eventmaps/medal.ts`     勋章系统事件
 *   - `eventmaps/rlv2.ts`      肉鸽V2 领域事件
 *   - `eventmaps/activity.ts`  限时活动事件
 *
 * 本文件将五个领域映射组合为统一的 `EventMap`，并保留全部运行时实现
 * （TypedEventEmitter / EventBus / globalEventBus / Priority / 中间件 / 验证器），
 * 对外保持单一入口 `@game/model/events`，调用方无需感知拆分。
 *
 * 新增事件：在对应领域文件中追加即可；若事件跨领域共享，放入 `core.ts`。
...
export { Priority } from "./eventmaps/priority";
 ...
 */
// 保持导出兼容：Priority 见 ./eventmaps/priority（此处类型层面由下方事件总线使用）
